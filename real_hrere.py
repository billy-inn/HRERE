...
class RealHRERE(Model):
    def __init__(self, sequence_length, num_classes, vocab_size, embedding_size,
            position_size, pretrained_embedding, wpe, entity_embedding,
            relation_embedding, hparams):
        super(RealHRERE, self).__init__(sequence_length, num_classes,
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("entity_output"):
            self.r = tf.Variable(self.relation_embedding, dtype=tf.float32,
                    name="relation_embedding")
            self.var_list1.append(self.r)
            self.entity_scores = tf.negative(tf.reduce_sum(tf.abs(
                tf.expand_dims(self.e1, 1) + tf.expand_dims(self.r, 0) -
                tf.expand_dims(self.e2, 1)), -1), name="entity_scores")
            self.entity_probs = tf.nn.softmax(self.entity_scores, name="entity_probs")
            self.entity_preds = tf.argmax(self.entity_scores, 1, name="entity_preds")
    ...
    def add_loss_op(self):
        with tf.name_scope("loss"):
            ...
            l2_loss1 = tf.reduce_mean(tf.square(self.e1)) + \
